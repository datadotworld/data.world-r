...
#' @export
downloadFile.default <- function(object, ...) {
  print("nope.")
}

#' Download file from the latest dataset version
#' @param apiClient the connection to data.world
...
#' @examples
#' downloadFile(data.world()$apiClient, dataset="ownerid/datasetid" , fileName = "file.csv",
#' output = "tmp/file.csv")
#' @export
downloadFile.ApiClient <- function(apiClient, dataset, fileName, output) {
  url = sprintf("%s/file_download/%s/%s", apiClient$baseDownloadApiUrl, dataset, fileName)

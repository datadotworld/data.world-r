...
set_default_demo_variables <- function() {
  ...
  if (exists("default_dataworld_user_id")) {
    demo_dataset_owner <- readline(sprintf("data.world user_id the current token belong to  (default to %s): ", default_dataworld_user_id))
  ...
  demo_dataset_title <- readline(sprintf("dataset title for this demo (default to %s) : ", default_dataset_title))

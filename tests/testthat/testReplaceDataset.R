...
test_that("replaceDataset making the correct HTTR request" , {
  request <- data.world::DatasetPutRequest(visibility = "OPEN", description = "UPDATED DESCRIPTION !")
  response <- with_mock(
    `httr::PUT` = function(url , body, header , userAgent)  {
      expect_equal(url,
    ...
    `mime::guess_type` = function(...)
      NULL,
    data.world::replaceDataset(
      data.world(token = "API_TOKEN"),
      "ownerid/datasetid",
      datasetPutRequest = request
    )
  )

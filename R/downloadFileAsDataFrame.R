...
#' @export
downloadFileAsDataFrame <- function(object, ... ) {
  UseMethod("downloadFileAsDataFrame")
}

#' @export
downloadFileAsDataFrame.default <- function(object, ... ) {
  print("nope.")
}
...
#' @export
downloadFileAsDataFrame.ApiClient <- function(apiClient, dataset, fileName) {
  ...
  download_status <- data.world::downloadFile(apiClient, dataset, fileName, tmp_path)
